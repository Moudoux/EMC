...
public class GuiUpdateLoader extends GuiScreen {
	...
	public void initGui() {
		buttonList.clear();
		buttonList.add(new GuiButton(0, width / 2 - 100, height / 4 + 120 + 12 - 30, "Update EMC"));
	...
	protected void actionPerformed(GuiButton clickedButton) throws IOException {
		if (clickedButton.id == 0) {
			try {
				Desktop.getDesktop().browse(new URL("https://github.com/Moudoux/EMC-Installer/releases").toURI());
			} catch (Exception e) {
				;
			}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		this.drawCenteredString(this.fontRendererObj, "EMC update required", this.width / 2, 21,
				16777215);
		...
		this.drawCenteredString(this.fontRendererObj, "Your EMC version is too low.",
				this.width / 2 + 70, 90, 16777215);
