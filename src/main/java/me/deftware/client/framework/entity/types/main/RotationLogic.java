...
public class RotationLogic extends EntityPlayer {
	...
	public boolean faceVectorClient(Vector3d vec) {
		...
		entity.yaw = limitAngleChange(oldYaw, rotations[0], 30);
		entity.pitch = rotations[1];
	...
	public float getAngleToClientRotation(Entity entity) {
		...
		float diffYaw = MathHelper.wrapDegrees(this.entity.yaw) - needed[0];
		float diffPitch = MathHelper.wrapDegrees(this.entity.pitch) - needed[1];
