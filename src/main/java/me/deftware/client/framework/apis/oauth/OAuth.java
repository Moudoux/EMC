    ...
    public static void oAuth(OAuthCallback callback) {
        new Thread(() -> {
            try {
                ...
                manager.sendPacket(new HandshakeServerPacket(OAuth.ip, OAuth.port, NetworkState.LOGIN));
                manager.sendPacket(new LoginHelloServerPacket(MinecraftClient.getInstance().getSession().getProfile()));
            } catch (Exception ex) {
